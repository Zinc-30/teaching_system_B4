Vue.component('demo-grid', {
  ...
  methods: {
    ...
    dropOn: function() {
        var ele = this;
        $.ajax({
            url: ele.prefixUrl + '/Admin/move',
            type: 'POST',
            data: {
                dragId: ele.data[ele.dragIndex].id,
                targetId: ele.data[ele.targetIndex].id,
            },
            dataType: 'JSON',
            success: function(res) {
                var tableData = ele.data;
                var dragRow = ele.data[ele.dragIndex];
                var newRecord = {name: dragRow.name, id: dragRow.id,
                    author_id: dragRow.author_id, author_name: dragRow.author_name,
                    duetime: dragRow.duetime, is_folder: dragRow.is_folder};
                ele.generatePic(newRecord);
                tableData.splice(ele.targetIndex, 0, newRecord);
    ...
    dragEnter: function(ev, index){
        var ele = this;
        ele.targetIndex = index;
        ev.preventDefault();
        return true;
    },
